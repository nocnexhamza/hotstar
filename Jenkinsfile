pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/nocnexhamza/hotstar.git'
            }
        }
            ...
            steps {
                script {
                    ...
                    withSonarQubeEnv('SonarQube') {
                        sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=hotstar -Dsonar.sources=. -Dsonar.language=js -Dsonar.exclusions=Dockerfile"
                    }
                }
            }
        }
